...
public class CountChain {
    ...
    public static final Map<Object,Object> handleLeave(Map<Object,Object> context,
        ...
        int stackIdx = (int)context.getOrDefault("dais.stackIdx", 0);

        IInterceptor[] stackArray = (IInterceptor[])context.get("dais.stackArray");
        if (stackArray == null) {
            stackArray = stack.toArray(new IInterceptor[0]);
            context.put("dais.stackArray", stackArray);
        }

        int length = stackArray.length;
        while (stackIdx < length) {
            IInterceptor interceptor = stackArray[stackIdx]; // Our 'stack' is already "reversed" so grab the head
            stackIdx++;
            ...
            try {
                ...
                if (context.get("error") != null) {
                    context.put("dais.stackIdx", stackIdx);
                    return handleError(context, interceptor, stack);
                }
            } catch (Throwable t) {
                context.put("error", t);
